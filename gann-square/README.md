...
7b. **风车跑图法**  
   ...
   - **阶段 2 · 骨架跑图**：「风车骨架跑图」→ 仅起点落在十字/对角骨架上时对穿逼近（方案 A：完整对穿，含如 133→20 的 31）；起点/目标超出当前环时**自动扩环**；起点不在骨架上暂不跑（叶区走线见阶段四）  
   - **阶段 3 · 叶区显示**：八叶区主名（北/东北…）+ 副名夹轨；轨迹端点标 1–4；点选区名或点格子高亮（不跑叶区路径）  
