...
纯静态网页：调起始值、步长、环数即可生成江恩九方图。支持价格/时间模式、角度高亮、价格反查、Constellate 跑图/推演、四角推图法、风车构图、手工批注画线、左右侧栏拖拽变宽、导出与分享链接。
...
7c. **四角推图法（第一版）**  
   - 与 Constellate **共用起点价 / 目标价**（以及 Begin / Step / Rings）  
   - 反复走 **45°**（对角、朝目标最近价差、更接近目标即停），折线**含起点**  
   - 起点只是寻猎原点，**不跑 180°**；每个 45° 落点再画一条同行/同列 180°（向下找更高反弹，不够高则对侧外扩；向上找更低回撤，不够低则对侧内收）  
   - 第一版：**两列表 + 两色画线**，不做推演段数  
   - 例向下（1/1，922→660）：`922 → 880 → … → 660`；180° `880→952` … `822→891` … `660→720`  
   - 例向上（1/1，660→922）：180° 回撤含 `748→685`、`766→721`、`862→793`、`880→833`
