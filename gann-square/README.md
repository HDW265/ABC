...
7c. **四角推图法（第一版）**  
   ...
   - 起点只是寻猎原点，**不跑 180°**；每个 45° 落点再画一条同行/同列 180° 反弹（向下找更高、向上找回撤；对点若不是反弹则沿该轴外扩 1 环）  
   - 第一版：**两列表 + 两色画线**，不做推演段数  
   - 例（1/1，922→660）：`922 → 880 → 862 → 822 → 804 → 766 → 748 → 712 → 694 → 660`；180° 为 `880→952` … `822→891` … `660→720`
